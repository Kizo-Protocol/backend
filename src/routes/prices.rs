...
async fn get_apt_usd_price(
    ...
    match price_feed.get_apt_usd_price().await {
        ...
        Err(e) => {
            ...
            Err(AppError::InternalError(format!("Failed to fetch price: {}", e)))
        }
    }
}
...
async fn refresh_apt_usd_price(
    State(_db): State<Database>,
) -> Result<Json<Value>, AppError> {
    ...
    match price_feed.refresh_price().await {
        ...
        Err(e) => {
            ...
            Err(AppError::InternalError(format!("Failed to refresh price: {}", e)))
        }
    }
}
