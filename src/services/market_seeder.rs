...
impl MarketSeeder {
    pub fn new(pool: PgPool, api_key: String) -> Result<Self> {
        ...
        info!("ℹ️  Market seeder will create markets in database only.");
        info!("ℹ️  Use the deploy_markets.sh script to deploy to blockchain.");
    ...
    pub async fn seed_markets(&self, count: usize) -> Result<SeedResult> {
        ...
        for market in api_response.data {
            ...
            if self.is_question_duplicate(&market.question, &seen_questions) {
                info!("⏭️  Skipping duplicate/similar question: {}", 
        ...
        info!("✅ Found {} unique markets after deduplication", unique_markets.len());
    ...
    async fn get_existing_questions(&self) -> Result<Vec<String>> {
        let questions = sqlx::query_scalar!(
            "SELECT question FROM markets_extended"
        )
        .fetch_all(&self.pool)
        .await?
        .into_iter()
    ...
    fn is_question_duplicate(&self, question: &str, existing: &[String]) -> bool {
        ...
        for existing_q in existing {
            ...
            let similarity = self.calculate_question_similarity(&normalized_question, &normalized_existing);
            if similarity > 0.8 { 
                return true;
            }
        }
    ...
    async fn create_or_update_market(&self, api_market: &AdjacentMarket) -> Result<bool> {
        ...
        let end_date = chrono::DateTime::parse_from_rfc3339(&api_market.end_date)?
            .naive_utc();
        ...
        let volume = BigDecimal::try_from(api_market.volume.unwrap_or(0.0))
            .unwrap_or_else(|_| BigDecimal::from(0));
        let open_interest = BigDecimal::try_from(api_market.open_interest.unwrap_or(0.0))
            .unwrap_or_else(|_| BigDecimal::from(0));
        ...
        let existing = sqlx::query!(
            "SELECT id FROM markets_extended WHERE \"adjTicker\" = $1",
        ...
        if let Some(existing_market) = existing {
            ...
            sqlx::query!(
                ...
                INSERT INTO markets_extended (
                    ...
                    result, link, "imageUrl", "createdAt", "updatedAt"
                )
            ...
            info!("ℹ️  Market created in database. Use deploy script to deploy to blockchain.");
