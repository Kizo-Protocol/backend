...
impl ImageService {
    ...
    pub async fn check_api_status(&self) -> Result<PexelsApiStatus> {
        if self.pexels_api_key.is_empty() {
            return Ok(PexelsApiStatus {
                is_available: false,
                error: Some("PEXELS_API_KEY not configured".to_string()),
            ...
            Ok(response) => {
                ...
                Ok(PexelsApiStatus {
                    ...
                    error: if !response.status().is_success() {
                        Some(format!("API returned status: {}", response.status()))
                    } else {
                        None
                    },
...
mod tests {
    ...
    fn test_clean_question() {
        let result = ImageService::clean_question_for_search("Will Bitcoin reach $100k?");
